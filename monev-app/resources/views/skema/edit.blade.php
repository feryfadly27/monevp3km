<x-app-layout>
    <x-slot name="pageTitle">Edit Skema</x-slot>
    <x-slot name="pageSubtitle">{{ $skema->nama }}</x-slot>

    <div class="max-w-2xl">
        <form method="POST" action="{{ route('skema.update', $skema->id) }}" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white border border-slate-200 rounded-[8px] p-6 space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="kategori_id" class="input-field" required>
                            @foreach($kategori as $kat)
                                <option value="{{ $kat->id }}" {{ old('kategori_id', $skema->kategori_id) == $kat->id ? 'selected' : '' }}>
                                    {{ $kat->nama }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="kode" value="{{ old('kode', $skema->kode) }}"
                               class="input-field font-mono uppercase @error('kode') border-red-400 @enderror" required>
                ...
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama', $skema->nama) }}"
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" name="dana_maksimal" value="{{ old('dana_maksimal', $skema->dana_maksimal) }}"
                    ...
                    <div>
                        ...
                        <input type="number" name="durasi_bulan" value="{{ old('durasi_bulan', $skema->durasi_bulan) }}"
                ...
                <div>
                    ...
                    <textarea name="deskripsi" rows="2" class="input-field resize-none">{{ old('deskripsi', $skema->deskripsi) }}</textarea>
                </div>

                <div class="flex items-center gap-2 pt-1">
                    <input type="checkbox" name="aktif" id="aktif" value="1" {{ old('aktif', $skema->aktif) ? 'checked' : '' }} class="rounded">
                    <label for="aktif" class="font-medium text-navy" style="font-size:14px;">Skema aktif</label>
                    <span class="text-slate-400" style="font-size:13px;">(nonaktif = tidak muncul di pilihan kegiatan baru)</span>
                </div>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-[8px] p-6">
                ...
                <p id="luaran-empty" class="text-slate-400 text-center py-4" style="font-size:14px; display:none;">
                    Belum ada target luaran.
                </p>
            </div>
    ...
    function addLuaran(data = {}) {
        ...
        div.innerHTML = `
            <div class="grid grid-cols-[120px_1fr_80px_auto] gap-3 flex-1 items-start">
                ...
                <label class="flex items-center gap-1.5 text-slate-600 mt-2 whitespace-nowrap" style="font-size:13px;">
                    <input type="checkbox" name="luaran[${i}][wajib]" ${data.wajib!==false?"checked":""} class="rounded">
                    Wajib
                </label>
            </div>
            <button type="button" onclick="removeLuaran(${i})"
                    class="mt-1 p-1 text-slate-400 hover:text-red-500 transition">
    ...
    // Pre-fill existing luaran
    const existingLuaran = @json($skema->luaran);
    existingLuaran.forEach(l => addLuaran({ jenis: l.jenis, deskripsi: l.deskripsi, jumlah_minimal: l.jumlah_minimal, wajib: l.wajib }));
    if (!existingLuaran.length) document.getElementById("luaran-empty").style.display = "block";
    </script>
</x-app-layout>
